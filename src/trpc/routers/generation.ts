...
export const generationsRouter=createTRPCRouter({
    ...
    .query(async({input,ctx})=>{
        ...
        if(!generation){
            throw new TRPCError({
                code:"NOT_FOUND",
                message:`Generation with id ${input.id} not found`,
            })
        }
        return {
            ...generation,
    ...
    .input(
        z.object({
            ...
            topK:z.number().min(0).default(1000),
...
.mutation(async({input,ctx})=>{
    ...
    if(!voice){
        throw new TRPCError({
            code:"NOT_FOUND",
            message:`Voice with id ${input.voiceId} not found`,
        });
    }  
    ...
    if(!voice.r2ObjectKey){
        throw new TRPCError({
            ...
            message:`Voice with id ${input.voiceId} is not ready for generation`,
        });
    }
