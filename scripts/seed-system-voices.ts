...
import { CANONICAL_SYSTEM_VOICE_SCOPES } from "../src/features/voices/data/voice-scoping";
import { z } from "zod";
import { PrismaPg } from "@prisma/adapter-pg";

const SYSTEN_VOICES_DIR = path.join(
...
const systemVoiceMetadata:Record<string, VoiceMetadata> = {
    ...
    Gavin:{
        description:"Calm and ressuring with a smooth, natural flow",
        category:"MEDITATION",
        language:"en-US"
    },
    Gordon:{
    ...
    Laura:{
        description:"Authentic and warm with a conversational Midewestern tone",
        category:"CONVERSATIONAL",
        language:"en-US"
    },
    Lucy:{
    ...
    Madison:{
        description:"Energetic and unfiltered with a causal, chatty vibe",
        category:"PODCAST",
        language:"en-US"
    },
    Marisol:{
        description:"Confident and polished with a persuasive, professional tone, ad-ready delivery",
        category:"ADVERTISING",
        language:"en-US"
    },
    Meera:{
        description:"Friendly and helpful with  clear, service-oriented tone",
        category:"CUSTOMER_SERVICE",
        language:"en-IN"
    },
    Walter:{
...
async function readSystemVoiceAudio(name:string){
    const filePath = path.join(SYSTEN_VOICES_DIR, `${name}.wav`);
...
async function main(){
    console.log(`Seeding ${CANONICAL_SYSTEM_VOICE_SCOPES.length} system voices...`);
    for(const name of CANONICAL_SYSTEM_VOICE_SCOPES){
